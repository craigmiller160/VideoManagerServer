...
class VMModelMapperIntegrationTest {
  ...
  fun test_map() {
    ...
    assertThat(
      result, allOf(isA(Test2::class.java), hasProperty("firstName", equalTo(first.firstName))))
  }
  ...
  fun test_mapExisting_categoryPayloadCategory() {
    val payload = CategoryPayload(categoryName = "Category")
    val existing = Category(categoryName = "Category2", hidden = true)
    val result = mapper.mapFromExisting(payload, existing)
    assertThat(
      result,
      allOf(
  ...
  class Test1(var firstName: String = "")

  class Test2(var firstName: String = "")
}
