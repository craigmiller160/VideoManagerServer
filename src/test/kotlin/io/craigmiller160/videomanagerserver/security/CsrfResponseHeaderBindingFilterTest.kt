...
class CsrfResponseHeaderBindingFilterTest {
  ...
  fun test_doFilterInternal() {
    ...
    `when`(token.token).thenReturn(tokenString)
    `when`(req.getAttribute(CSRF_REQ_ATTR_NAME)).thenReturn(token)

    csrfResponseHeaderBindingFilter.doFilterInternal(req, resp, chain)

    verify(resp, times(1)).setHeader(RESPONSE_TOKEN_NAME, tokenString)
