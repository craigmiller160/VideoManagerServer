...
class StarServiceTest {
  ...
  companion object {
    ...
    private val expectedStars =
      listOf(Star(starId = 1, starName = FIRST_NAME), Star(starId = 2, starName = SECOND_NAME))

    private val expectedStarPayloads =
      listOf(
        StarPayload(starId = 1, starName = FIRST_NAME),
        StarPayload(starId = 2, starName = SECOND_NAME))
  }

  @Mock private lateinit var starRepo: StarRepository
  @Mock private lateinit var fileStarRepo: FileStarRepository
  @InjectMocks private lateinit var starService: StarService
  ...
  @Test
  fun testGetStar() {
    Mockito.`when`(starRepo.findById(1)).thenReturn(Optional.of(expectedStars[0]))
    Mockito.`when`(starRepo.findById(2)).thenReturn(Optional.of(expectedStars[1]))
  ...
  fun testUpdateStar() {
    ...
    Mockito.`when`(starRepo.save(newStarWithId)).thenReturn(newStarWithId)
    Mockito.`when`(starRepo.findById(1)).thenReturn(Optional.of(expectedStars[0]))
  ...
  fun test_deleteStar() {
    ...
    Mockito.verify(starRepo, Mockito.times(2)).deleteById(1)
    Mockito.verify(fileStarRepo, Mockito.times(2)).deleteAllByStarId(1)
  }
}
