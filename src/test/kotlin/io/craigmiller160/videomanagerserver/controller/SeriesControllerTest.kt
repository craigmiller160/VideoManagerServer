...
class SeriesControllerTest : AbstractControllerTest() {
  ...
  @MockBean private lateinit var seriesService: SeriesService

  @Autowired private lateinit var seriesController: SeriesController
  ...
  fun testGetAllSeries() {
    ...
    `when`(seriesService.getAllSeries()).thenReturn(seriesList).thenReturn(listOf())
  ...
  fun testGetSeries() {
    mockMvcHandler.token = token
    `when`(seriesService.getSeries(1)).thenReturn(series1)
    `when`(seriesService.getSeries(5)).thenReturn(null)
  ...
  fun testAddSeries() {
    ...
    `when`(seriesService.addSeries(seriesNoId)).thenReturn(seriesWithId)
  ...
  fun testUpdateSeries() {
    ...
    `when`(seriesService.updateSeries(1, series2)).thenReturn(updatedSeries)
    `when`(seriesService.updateSeries(5, series3)).thenReturn(null)
  ...
  fun testDeleteSeries() {
    mockMvcHandler.token = editToken
    `when`(seriesService.deleteSeries(1)).thenReturn(series1).thenReturn(null)
