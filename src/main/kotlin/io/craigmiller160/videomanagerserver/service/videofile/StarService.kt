...
class StarService(
  ...
  fun getAllStars(): List<StarPayload> {
    val sort = Sort.by(Sort.Order(Sort.Direction.ASC, "starName", Sort.NullHandling.NULLS_LAST))
    return starRepo.findAll(sort).map { star -> modelMapper.map(star, StarPayload::class.java) }
  }
  ...
  fun deleteStar(starId: Long): StarPayload? {
    ...
    return starOptional.map { star -> modelMapper.map(star, StarPayload::class.java) }.orElse(null)
  }
}
